# stats.py Results-Visualization Module Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add `src/cl3o/utils/stats.py`, a standalone plots-only consumer that reads the LHS DE sweep, ANOVA sensitivity tables, and a run's last-generation `.pkl`, and renders vector-PDF figures into `tools/output/stats/`.

**Architecture:** Single module following the project's Data / Helper / Main 3-layer convention. `StatsData` (config + derived paths), `StatsHelper` (pandas/pickle loaders + a scipy Spearman block, all static), `RunStats` (orchestrator with `plot_lhs` / `plot_sensitivity` / `plot_best_design` / `run_all`). All inputs are injectable via `StatsData` fields so tests need no monkeypatching. Missing-artifact sources are skipped with a warning rather than raising.

**Tech Stack:** Python 3.10+, pandas, numpy, scipy (`spearmanr`), matplotlib + seaborn (new optional `[analysis]` extra). Headless rendering via `MPLBACKEND=Agg` (already set in `tests/conftest.py`).

---

## File Structure

- **Create** `src/cl3o/utils/stats.py` — the whole module (Data / Helper / Main).
- **Create** `tests/test_stats.py` — unit tests for `StatsData` path derivation, `StatsHelper` loaders, and headless plotting smoke tests.
- **Modify** `pyproject.toml` — add `[project.optional-dependencies] analysis = ["seaborn>=0.13"]`.

Reference (read-only) for access patterns and conventions:
- `src/cl3o/ui/backend/extract.py` — `stress`/`forces`/`info` show how to read `rt.stress`, `rt.fea_rts`, `rt.mesh`.
- `tools/sensitivity_analysis.py:137-177` — the `_PKL_SUBDIRS` last-pkl search logic to mirror.
- `src/cl3o/utils/io_utils.py` — `setup_logger`.
- `src/cl3o/paths.py` — `ROOT_DIR`, `OUTPUTS_DIR`.

Field names confirmed in the codebase (use exactly these):
- `rt.score`: `.total: float`, `.panels: np.ndarray`, `.flanges: np.ndarray`.
- `rt.tsw`:   `.MS_panels`, `.MS_booms`, `.MS_min: float`.
- `rt.displ`: `.MS_u`, `.MS_th`, `.MS_min: float`.
- `rt.stress`:`.tauA`, `.tauB` (shape `(m, 10, nc)`).
- `rt.fea_rts`:`.Q_sc`, `.Q_c` (shape `(12, m, nc)`).
- `rt.mesh`:  `.coord` `(n, 3)`, `.conn` `(m, >=2)`.
- `rt.sections.sec_data[0]`: has attribute `T2` (list of dicts with `.get("label")`).

---

## Task 1: Add the seaborn optional dependency

**Files:**
- Modify: `pyproject.toml:33-38` (the `[project.optional-dependencies]` block)

- [ ] **Step 1: Add the `analysis` extra**

In `pyproject.toml`, immediately after the `desktop = [...]` extra (around line 41-42), add:

```toml
# Post-hoc results analysis / visualization (cl3o.utils.stats).
analysis = [
    "seaborn>=0.13",
]
```
...
Expected: seaborn (and its deps) install; `pip show seaborn` prints a version `>= 0.13`.
...
git commit -m "build: add seaborn as optional [analysis] extra"
```

---

## Task 2: Module skeleton + `StatsData`

**Files:**
- Create: `src/cl3o/utils/stats.py`
...
Run: `pytest tests/test_stats.py::test_statsdata_path_derivation -v`
Expected: FAIL with `ModuleNotFoundError: No module named 'cl3o.utils.stats'`.

- [ ] **Step 3: Write the module skeleton + `StatsData`**
...
```bash
git add src/cl3o/utils/stats.py tests/test_stats.py
git commit -m "feat(stats): module skeleton and StatsData config"
```

---

## Task 3: `StatsHelper` loaders + Spearman block

**Files:**
- Modify: `src/cl3o/utils/stats.py` (append `StatsHelper`)
- Test: `tests/test_stats.py` (append loader tests)
...
Run: `pytest tests/test_stats.py -k "spearman or rate_curves or last_pkl or anova" -v`
Expected: FAIL with `AttributeError: ... has no attribute 'StatsHelper'` (or `spearman_matrix`).

- [ ] **Step 3: Implement `StatsHelper`**

Append to `src/cl3o/utils/stats.py`:
...
```bash
git add src/cl3o/utils/stats.py tests/test_stats.py
git commit -m "feat(stats): StatsHelper loaders and Spearman block"
```

---

## Task 4: `RunStats` scaffold + LHS figures

**Files:**
- Modify: `src/cl3o/utils/stats.py` (append `RunStats` with `plot_lhs` + private LHS plotters + `_save`)
- Test: `tests/test_stats.py` (append LHS smoke test)
...
Run: `pytest tests/test_stats.py::test_plot_lhs_writes_pdfs -v`
Expected: FAIL with `ImportError`/`AttributeError` (no `RunStats`).

- [ ] **Step 3: Implement `RunStats` scaffold + LHS plotters**
...
Run: `pytest tests/test_stats.py::test_plot_lhs_writes_pdfs -v`
Expected: PASS (4 PDFs created).

- [ ] **Step 5: Commit**

```bash
git add src/cl3o/utils/stats.py tests/test_stats.py
git commit -m "feat(stats): RunStats scaffold and LHS figures"
```

---

## Task 5: Sensitivity (ANOVA) figures

**Files:**
- Modify: `src/cl3o/utils/stats.py` (append `plot_sensitivity` + private plotters)
- Test: `tests/test_stats.py` (append sensitivity smoke test)
...
Run: `pytest tests/test_stats.py::test_plot_sensitivity_writes_pdfs -v`
Expected: FAIL with `AttributeError: 'RunStats' object has no attribute 'plot_sensitivity'`.

- [ ] **Step 3: Implement the sensitivity plotters**

Append inside the `RunStats` class in `src/cl3o/utils/stats.py` (after `_lhs_speed_ecdf`):
...
Run: `pytest tests/test_stats.py::test_plot_sensitivity_writes_pdfs -v`
Expected: PASS (2 PDFs created).

- [ ] **Step 5: Commit**

```bash
git add src/cl3o/utils/stats.py tests/test_stats.py
git commit -m "feat(stats): ANOVA sensitivity figures"
```

---

## Task 6: Best-design figures (last `.pkl`)

**Files:**
- Modify: `src/cl3o/utils/stats.py` (append `plot_best_design` + private plotters)
- Test: `tests/test_stats.py` (append a slow smoke test using the `runtime` fixture)
...
Run: `pytest tests/test_stats.py::test_plot_best_design_writes_pdfs -v`
Expected: FAIL with `AttributeError: 'RunStats' object has no attribute 'plot_best_design'`.

- [ ] **Step 3: Implement the best-design plotters**

Append inside the `RunStats` class in `src/cl3o/utils/stats.py` (after `_anova_group_means`):
...
Run: `pytest tests/test_stats.py::test_plot_best_design_writes_pdfs -v`
Expected: PASS (4 PDFs created). This builds the session-scoped `runtime` fixture, so it is slower.

- [ ] **Step 5: Commit**

```bash
git add src/cl3o/utils/stats.py tests/test_stats.py
git commit -m "feat(stats): best-design figures from last pkl"
```

---

## Task 7: `run_all` + `__main__` + final verification

**Files:**
- Modify: `src/cl3o/utils/stats.py` (append `run_all` + `__main__`)
- Test: `tests/test_stats.py` (append a `run_all` smoke test on partial data)
...
Run: `pytest tests/test_stats.py::test_run_all_partial_data -v`
Expected: FAIL with `AttributeError: 'RunStats' object has no attribute 'run_all'`.

- [ ] **Step 3: Implement `run_all` and the `__main__` block**

Append inside the `RunStats` class (after `_design_forces`):
...
Then append the module-level entry point at the very end of `src/cl3o/utils/stats.py`:
...
- [ ] **Step 5: Run the whole fast test file**

Run: `pytest tests/test_stats.py -m "not slow" -v`
Expected: all non-slow tests PASS.

- [ ] **Step 6: End-to-end smoke against real artifacts**

Run (PowerShell): `$env:MPLBACKEND="Agg"; python -m cl3o.utils.stats`
Expected: figures appear in `tools/output/stats/` — at minimum the four `lhs_*.pdf` and the four `design_*.pdf` (the `tune-de-3` sweep + its `LHS-0` pkl exist). The two `anova_*.pdf` are skipped with a `[CL3O]` warning unless the variant-A `anova_results.csv` + `anova_summary.csv` have already been produced.

- [ ] **Step 7: Import check**

Run: `python -c "import cl3o.utils.stats"`
Expected: no error (confirms seaborn present and the module imports clean).

- [ ] **Step 8: Commit**

```bash
git add src/cl3o/utils/stats.py tests/test_stats.py
git commit -m "feat(stats): run_all orchestration and CLI entry point"
```
...
- **Spec coverage:** §4.1 StatsData (Task 2); §4.2 StatsHelper incl. all five loaders + Spearman (Task 3); §4.3 RunStats orchestrator (Tasks 4-7); §5.1 four LHS figures (Task 4); §5.2 two ANOVA figures (Task 5); §5.3 four best-design figures (Task 6); §6 seaborn `[analysis]` extra (Task 1); §7 graceful-skip error handling (Tasks 4-7, verified Task 7 Step 4); §8 verification (Task 7 Steps 5-7). All sections mapped.
- **PDF format (note "save all plots in .pdf"):** every `_save` call writes `<name>.{fmt}` with `fmt="pdf"`; smoke tests assert `.pdf` files. Covered.
- **No producer edits:** plan never touches `tools/tune_de.py` or `tools/sensitivity_analysis.py`. The variant-A ANOVA CSVs are an external precondition (Task 7 Step 6 note), per the user's "I'll redefine it first" decision.
- **Type/name consistency:** `StatsData` derived fields (`lhs_results`, `anova_results`, `anova_summary`, `rate_pattern`, `outputs_root`, `out_dir`, `fmt`) are referenced consistently across all tasks; `StatsHelper.load_*` signatures match their call sites; `_LHS_PARAMS`/`_LHS_OUTCOMES` defined in Task 2 and used in Task 4.
