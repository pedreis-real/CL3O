...
# Geometry data container (defined in geom_data; re-exported for compatibility)
from cl3o.geometry.geom_data import GeomData
...
class GeomPropCalculator:
    ...
    def _segment_T2(self):
        ...
        def _r(
            ...
            # Per-segment lengths, cached for reuse in
            # _precompute_panel_moments (avoids recomputing the same diffs
            # there). The panel arc length is s = sum(ds).
            dxv = np.diff(xx)
            dzv = np.diff(zz)
            ds  = np.sqrt(dxv * dxv + dzv * dzv)
            self._panel_ds.append(ds)
            return {
                'label'  : label,
                'pts'    : pts,
        ...
        self._panel_ds = []        # per-panel segment lengths, filled by _r
        self.T2 = [
    ...
    def _precompute_panel_moments(self):
        ...
        panel_ds = self._panel_ds      # cached per-segment lengths from _r
        for i, r in enumerate(self.T2):
            ...
            Sx2[i] = float(np.dot(xm * xm, dA))
            Sz2[i] = float(np.dot(zm * zm, dA))
    ...
    def _apply_structural_idealization(self) -> None:
        '''Steps 11-12: boom areas + idealized inertia (StructuralIdealization).'''
    ...
    def run(self) -> GeomData:
        ...
        self._apply_structural_idealization()
